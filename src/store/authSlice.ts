...
import { clearAllCookies, COOKIE_CONFIG, getCookie, setCookie } from "../utils/cookie";
 ...
 reducers: {
  loginUser: (state, action: PayloadAction<LoginPayload>) => {
   ...
   setCookie({
    ...
    expiresIn: action.payload.isManpowerLoggedIn === false ? undefined : COOKIE_CONFIG.accessTokenExpiryDuration,
   });
   setCookie({
    cookieName: COOKIE_CONFIG.refreshToken,
    value: action.payload.refreshToken,
    expiresIn: action.payload.isManpowerLoggedIn === false ? undefined : COOKIE_CONFIG.refreshTokenExpiryDuration,
   });
  },
